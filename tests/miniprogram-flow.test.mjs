...
test('upload confirmation preserves offline work and routes online work through private upload', () => {
  ...
  assert.match(confirm, /AssessmentClient\.cancelAssessment/)
  assert.match(confirm, /rotateCancelledDraft/)
  assert.match(confirm, /createLocalId\('idem'\)/)
  assert.match(confirm, /TaskMediaStore\.removeByLocalTaskId/)
  assert.match(confirm, /completed', 'partially_completed/)
  assert.match(confirm, /pages\/progress\/index/)
})

test('practice automatically resumes a retained local task when network connectivity returns', () => {
  const practice = read('miniprogram/pages/practice/index.ts')
  const network = read('miniprogram/services/network-service.ts')
  const declarations = read('miniprogram/types/wx.d.ts')
  assert.match(practice, /observeNetworkRecovery/)
  assert.match(practice, /void this\.resumePending\(\)/)
  assert.match(practice, /stopNetworkRecoveryObservation\?\.\(\)/)
  assert.match(network, /wx\.onNetworkStatusChange/)
  assert.match(network, /result\.isConnected/)
  assert.match(network, /wx\.offNetworkStatusChange/)
  assert.match(declarations, /onNetworkStatusChange/)
  assert.match(declarations, /offNetworkStatusChange/)
})

test('expired offline tasks are removed with their saved media under the same 30-day policy', () => {
  const app = read('miniprogram/app.ts')
  const localTasks = read('miniprogram/services/local-task-store.ts')
  assert.match(localTasks, /const RETENTION_MS = 30 \* 24 \* 60 \* 60 \* 1000/)
  assert.match(localTasks, /pruneExpired\(now = Date\.now\(\), protectedPaths/)
  assert.match(localTasks, /protectedPaths = new Set<string>\(\)/)
  assert.match(localTasks, /activePaths\(now = Date\.now\(\)\)/)
  assert.match(localTasks, /!Number\.isFinite\(createdAt\)/)
  assert.match(app, /LocalTaskStore\.activePaths\(now\)/)
  assert.match(app, /TaskMediaStore\.activePaths\(now\)/)
  assert.match(app, /LocalTaskStore\.pruneExpired\(now, taskActivePaths\)/)
  assert.match(app, /TaskMediaStore\.pruneExpired\(now, localActivePaths\)/)
  assert.match(app, /MediaService\.removeSaved\(path\)/)
})
...
test('result comparison uses only retained local media and service polygons', () => {
  ...
  assert.match(resultPage, /differenceAnnotations/)
  assert.match(resultPage, /difference-marker/)
  assert.match(resultPage, /annotation-legend/)
