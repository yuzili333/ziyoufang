...
- 关联原型：`../prototypes/mobile-v1.md`；正式评审记录为 `../reviews/prototype-review-2026-08-07.md`
...
本记录批准技术方向，不等同于三端能力验证通过。产品方随后已明确授权创建正式 `client/` 工程并同步开展能力验证；能力验证继续控制发布研发门禁。
...
- `client/` 已作为 ArkUI-X 正式工程根目录创建；公共 ArkTS 代码和 Android、iOS 平台宿主按 ArkUI-X 官方模板结构组织。
- `harness` 保持产品规则和验收场景的来源；客户端实现不得反向改写业务规则。
- 原型评审已批准、技术决策已接受；客户端能力验证 `approved` 和隐私合规批准后，方可解除发布研发门禁。
...
2. 完成未成年人数据、标准字形许可证和评测服务数据边界评审。
3. 能力验证与隐私合规门禁全部通过后，更新发布就绪状态。
