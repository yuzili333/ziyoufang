...
- 门禁影响：允许进入 `implementation-validation` 并创建微信小程序、云函数/BFF 与智能评测服务正式工程。隐私合规、授权验证集、生产阈值、供应商、真机和发布门禁仍独立有效；冻结的 ArkUI-X `client/` 在契约迁移完成并验证前暂不删除。
...
| SCN-004 字形对比 | P-06 | 叠加/并排已验证 | 标准宋体许可证 |
