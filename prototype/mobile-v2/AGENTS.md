...
- Visual direction: selected option 2, centered on a large single-character overlay comparison.
- Theme: copybook paper white, vermilion, and ink black; keep the product mark compact and prioritize the working area.
- Mobile navigation: `练习—拍照—我的`, with the vermilion capture button centered; `字本` is a secondary menu under `我的`.
- Evaluation dimensions: `笔画规范`, `间架结构`, `字形比例`, `位置布局`, and `稳定性`.
- Stability: show sample collection before three comparable practices; from the third comparable practice onward show a growth curve and monitoring status.
- Monitoring: low recent-average or low-stability characters enter `我的—字本—重点字库`; POC thresholds are 70 to enter and 80 to exit, pending expert calibration.
- Explicit exclusions: teacher review, weekly/monthly/quarterly/yearly reports, rankings, points, and challenges.
- This directory remains a clickable prototype with fixed mock data. It must not call production login, cloud storage, OCR, image analysis, or model services.
